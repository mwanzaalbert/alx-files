...
describe('+ FilesController', () => {
  ...
  const mockUser = {
    email: 'dumbledore@hogwarts.wizardry',
    password: 'dumbledores_got_style',
  };
  /**
   * 3 sample files
   * + 1 -> file
  ...
  describe('+ POST: /files', () => {
    ...
    it('+ Fails if type is missing', function (done) {
      ...
      request.post('/files')
        .set('X-Token', token)
        .send({ name: 'wizard_world_movies.txt' })
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
            return done(requestErr);
    ...
    it('+ Fails if type is available but unrecognized', function (done) {
      ...
      request.post('/files')
        .set('X-Token', token)
        .send({ name: 'wizard_world_movies.txt', type: 'nakamura' })
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
            return done(requestErr);
